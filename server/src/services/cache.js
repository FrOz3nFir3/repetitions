...
class CacheService {
    ...
    async checkRedisConnection() {
        ...
        try {
            ...
            console.log(`Redis connection check failed (attempt ${this.connectionErrors}):`, error.message);
    ...
    logStatistics() {
        ...
        console.log('[CACHE STATS]', {
            ...
            errors: stats.errors,
            compressionSaved: `${(stats.compressionSaved / 1024).toFixed(2)} KB`,
    ...
    async compressData(data) {
        ...
        try {
            ...
            this.logError(CACHE_ERRORS.SERIALIZATION_ERROR, 'compressData', error, { dataSize });
            return { data, compressed: false };
        }
    }
        ...
        try {
            const value = await Promise.race([
                ...
                new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('Cache read timeout')), CACHE_CONFIG.TIMEOUTS.READ)
                )
            ]);
            ...
            try {
                ...
                if (parsedValue.__compressed) {
                    const decompressed = await this.decompressData(parsedValue.data, true);
                ...
                this.logError(CACHE_ERRORS.SERIALIZATION_ERROR, 'get', parseError, { key });
                this.stats.errors++;
                return null;
            }
        } catch (error) {
    ...
    async set(key, value, ttl, tags = []) {
        ...
        try {
            ...
            try {
                ...
                this.logError(CACHE_ERRORS.SERIALIZATION_ERROR, 'set', serializeError, { key });
                this.stats.errors++;
                return false;
            }
            ...
            await Promise.race([
                ...
                new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('Cache write timeout')), CACHE_CONFIG.TIMEOUTS.WRITE)
                )
            ]);
            ...
            if (CACHE_CONFIG.DEBUG.ENABLED) {
                const sizeKB = (Buffer.byteLength(finalValue, 'utf8') / 1024).toFixed(2);
                console.log(`[CACHE SET] ${key} (${sizeKB}KB, TTL: ${ttl}s${tags.length > 0 ? `, tags: ${tags.length}` : ''})`);
            }
    ...
    async del(key) {
        ...
        try {
            ...
            const result = await Promise.race([
                redis.del(key),
                new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('Cache delete timeout')), CACHE_CONFIG.TIMEOUTS.DELETE)
                )
            ]);
    ...
    async exists(key) {
        ...
        try {
            const result = await Promise.race([
                redis.exists(key),
                new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('Cache exists timeout')), CACHE_CONFIG.TIMEOUTS.READ)
                )
            ]);
    ...
    async addTagsToKey(key, tags) {
        ...
        try {
            ...
            tags.forEach(tag => {
                ...
                pipeline.expire(tagKey, Math.max(...Object.values(CACHE_CONFIG.TTL)) + 3600); // Max TTL + 1 hour buffer
            });

            await Promise.race([
                pipeline.exec(),
                new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('Tag operation timeout')), CACHE_CONFIG.TIMEOUTS.WRITE)
                )
            ]);

            return true;
        } catch (error) {
            this.logError(CACHE_ERRORS.TAG_OPERATION_FAILED, 'addTagsToKey', error, { key, tags });
            return false;
        }
    }
    ...
    async removeTagsFromKey(key, tags) {
        ...
        try {
            ...
            this.logError(CACHE_ERRORS.TAG_OPERATION_FAILED, 'removeTagsFromKey', error, { key, tags });
            return false;
        }
    }
    ...
    async getKeysByTag(tag) {
        ...
        try {
            ...
            const keys = await Promise.race([
                ...
                new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('Tag lookup timeout')), CACHE_CONFIG.TIMEOUTS.READ)
                )
            ]);

            return keys || [];
        } catch (error) {
            this.logError(CACHE_ERRORS.TAG_OPERATION_FAILED, 'getKeysByTag', error, { tag });
            return [];
        }
    }
    ...
    async getTagsForKey(key) {
        ...
        try {
            ...
            this.logError(CACHE_ERRORS.TAG_OPERATION_FAILED, 'getTagsForKey', error, { key });
            return [];
        }
    }
    ...
    async mget(keys) {
        ...
        try {
            const values = await Promise.race([
                redis.mGet(keys),
                new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('Batch read timeout')), CACHE_CONFIG.TIMEOUTS.READ)
                )
            ]);
            ...
            keys.forEach((key, index) => {
                if (values[index] !== null) {
                    try {
                        ...
                        this.logError(CACHE_ERRORS.SERIALIZATION_ERROR, 'mget', parseError, { key });
                        result[key] = null;
                    }
                } else {
                    result[key] = null;
        ...
        try {
            ...
            Object.entries(keyValuePairs).forEach(([key, value]) => {
                try {
                    ...
                    this.logError(CACHE_ERRORS.SERIALIZATION_ERROR, 'mset', serializeError, { key });
                }
            });
            ...
            Object.entries(tagMap).forEach(([key, tags]) => {
                if (Array.isArray(tags) && tags.length > 0) {
                    tags.forEach(tag => {
                        const tagKey = `${CACHE_CONFIG.PREFIXES.TAG}:${tag}`;
                        pipeline.sAdd(tagKey, key);
                        pipeline.expire(tagKey, Math.max(...Object.values(CACHE_CONFIG.TTL)) + 3600);
                    });
                }
            });
            ...
            await Promise.race([
                pipeline.exec(),
                new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('Batch write timeout')), CACHE_CONFIG.TIMEOUTS.WRITE)
                )
            ]);
            ...
            const errorType = error.message.includes('timeout')
                ? CACHE_ERRORS.WRITE_TIMEOUT
                : CACHE_ERRORS.WRITE_FAILED;
            this.logError(errorType, 'mset', error, { keyCount: Object.keys(keyValuePairs).length, ttl });
            return false;
        }
    }
    ...
    async mdel(keys) {
        ...
        try {
            ...
            await Promise.race([
                pipeline.exec(),
                new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('Batch delete timeout')), CACHE_CONFIG.TIMEOUTS.DELETE)
                )
            ]);
    ...
    async invalidateByTags(tags) {
        ...
        try {
            ...
            const results = await Promise.race([
                pipeline.exec(),
                new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('Tag invalidation timeout')), CACHE_CONFIG.TIMEOUTS.DELETE)
                )
            ]);
            ...
            if (CACHE_CONFIG.DEBUG?.ENABLED) {
                console.log(`[CACHE INVALIDATION] ${deletedCount} keys invalidated for tags: [${tags.join(', ')}]`);
                if (keysArray.length > 0) {
                    console.log(`[CACHE INVALIDATION] Sample keys:`, keysArray.slice(0, 3));
                }
            }

            return deletedCount;
        } catch (error) {
            this.logError(CACHE_ERRORS.INVALIDATION_FAILED, 'invalidateByTags', error, { tags });
    ...
    async atomicUpdate(key, value, ttl, tags = [], tagsToInvalidate = []) {
        ...
        try {
            ...
            try {
                ...
                this.logError(CACHE_ERRORS.SERIALIZATION_ERROR, 'atomicUpdate', serializeError, { key });
                return false;
            }
            ...
            await Promise.race([
                pipeline.exec(),
                new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('Atomic update timeout')), CACHE_CONFIG.TIMEOUTS.WRITE)
                )
            ]);

            if (CACHE_CONFIG.STATS.ENABLED) {
                console.log(`Atomic update: ${key} (invalidated tags: [${tagsToInvalidate.join(', ')}])`);
            }
            ...
            const errorType = error.message.includes('timeout')
                ? CACHE_ERRORS.WRITE_TIMEOUT
                : CACHE_ERRORS.WRITE_FAILED;
            this.logError(errorType, 'atomicUpdate', error, { key, ttl, tags, tagsToInvalidate });
            return false;
        }
    }
        ...
        try {
            ...
            if (tagKeys.length > 0) {
                ...
                await Promise.race([
                    pipeline.exec(),
                    new Promise((_, reject) =>
                        setTimeout(() => reject(new Error('Tag cleanup timeout')), CACHE_CONFIG.TIMEOUTS.WRITE)
                    )
                ]);
            }
        } catch (error) {
            this.logError(CACHE_ERRORS.TAG_OPERATION_FAILED, 'removeKeyFromAllTags', error, { key });
        }
    }
    ...
    async warmCache(warmers) {
        if (!this.isRedisAvailable || !Array.isArray(warmers) || warmers.length === 0) {
        ...
        console.log(`[CACHE WARMING] Starting cache warming for ${warmers.length} entries...`);
            ...
            if (cached !== null) {
                ...
                if (remainingTTL > 0 && remainingTTL < refreshThreshold) {
                    ...
                    this.refreshCache(key, fetcher, ttl, tags).catch(error => {
                        console.log(`[CACHE REFRESH] Background refresh failed for ${key}:`, error.message);
                    });
                }
